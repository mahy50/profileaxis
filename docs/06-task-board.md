...
| P0-012 | 建立 apps/web 编辑器壳层 | Vue 3 + Pinia + Babylon 集成 + IndexedDB | P0-007, P0-008, P0-011 | `packages/render-babylon + bom + api` | `apps/web/src/stores/`、`apps/web/src/features/editor/` | Scene / 结构树 / 选择同步可用 | completed |
| P0-013 | 命令系统 | 语义命令、撤销/重做、快照恢复 | P0-003 | `packages/domain` | command bus、persistence store | 20 次编辑内历史一致；snapshot restore 后全部一致 | completed |
...
| P1-005 | catalog ETL 工具 | catalog CSV/XLSX → 规范化 JSON | P0-004, P0-002 | 新 catalog 可校验并产出 canonical JSON | completed |
